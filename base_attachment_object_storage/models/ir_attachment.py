...
class IrAttachment(models.Model):
        ...
        with self.do_in_new_env(new_cr=new_cr) as new_env:
            ...
            for attachment_id in attachment_ids:
                ...
                if current % 100 == 0 or total - current == 0:
                    _logger.info(
                        f"attachment {current}/{total} after "
    ...
    def _force_storage_to_object_storage(self, new_cr=False):
        ...
        # /odoo/addons/base/ir/ir_attachment.py#L344-L347  # noqa: B950
        ...
        with self.do_in_new_env(new_cr=new_cr) as new_env:
            ...
            for attachment_id in ids:
                try:
                    ...
                    _logger.error(f"Could not migrate attachment {attachment_id} to S3")
    ...
    def _to_http_stream(self):
        self.ensure_one()
        # If the attachment is stored in an object store (e.g. swift://...),
        # _to_http_stream() from core Odoo will try os.stat() on store_fname
        # as if it were a local path, which fails. We intercept here and
        # return an in-memory stream built from _file_read() instead.
        if self.store_fname and self._is_file_from_a_store(self.store_fname):
            ...
            if not data:
                raise FileNotFoundError(
                    f"[Errno 2] No such file or directory: '{self.store_fname}'"
                )
            return Stream(
                type="data",
                data=data,
